...
mod test_input_validations {
    use passman::utils::validation::{validate_arg, validate_password, validate_password_strength};
    ...
    #[test]
    fn validate_passwords_strength() {
        // valid passwords
        assert!(validate_password_strength("Test123!").is_ok());

        // invalid passwords
        assert!(validate_password_strength("").is_err());
        assert!(validate_password_strength("test").is_err());
        assert!(validate_password_strength("test123").is_err());
        assert!(validate_password_strength("Test123").is_err());
        assert!(validate_password_strength("test123!").is_err());
    }
}